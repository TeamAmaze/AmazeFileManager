	...
	//	Log.i("Amaze", "Extracting: " + entry);
		...
		try {
			...
			while ((len = inputStream.read(buf)) > 0) {
				...
				if(hash.get(id)){publishResults(true);	
				outputStream.write(buf, 0, len);
			
			}else{stopSelf(id);publishResults(false);}}
		} finally {
			outputStream.close();
			inputStream.close();
		}
	...
	public boolean extract(int id,File archive, String destinationPath) {
        ...
        try {
            ...
            for (Enumeration e = zipfile.entries(); e.hasMoreElements();) {
                ...
                i++;publishResults(id,archive.getName(),i*100/fileCount,false);
				}else{stopSelf(id);publishResults(false);}
...
public void onDestroy(){unregisterReceiver(receiver1);}
    /**
