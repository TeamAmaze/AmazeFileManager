...
class zip{
	...
	public void execute(int id,ArrayList<File> a,String fileOut){
		for(File f1:a){
			if(f1.isDirectory()){totalBytes=totalBytes+new Futils().folderSize(f1);}
			else{totalBytes=totalBytes+f1.length();}
		}
	...
	File 	zipDirectory = new File(fileOut);
		...
		ZipOutputStream zos;	private int isCompressed = 0;
...
public IBinder onBind(Intent arg0) {
	...
	return null;
			...
			@Override
	public void onDestroy(){this.unregisterReceiver(receiver1);
	}
}
