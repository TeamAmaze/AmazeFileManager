...
open class FtpHybridFileTest {
    ...
    protected open val ftpUrl: String
        get() = NetCopyClientUtils.encryptFtpPathAsNecessary(
            "${ftpPrefix}$USERNAME:$PASSWORD@${getLoopbackAddress().hostAddress}:$ftpPort"
        )
    ...
    fun testFileIO() {
        ...
        f.getInputStream(AppConfig.getInstance())?.run {
            ...
            await().atMost(10, TimeUnit.SECONDS).until {
                Arrays.equals(randomBytes, verify)
            }
    ...
    protected open fun createConnection(): NetCopyClient? {
        return FtpConnectionPool.getConnection(ftpUrl)
    }
    ...
    protected open fun createDefaultFtpServerListener(): Listener {
        return ListenerFactory().apply {
            port = PORT
            serverAddress = getLoopbackAddress().hostAddress
        }.createListener()
    }
}
