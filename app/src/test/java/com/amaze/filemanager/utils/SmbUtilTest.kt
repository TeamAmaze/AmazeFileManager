...
import com.amaze.filemanager.file_operations.filesystem.DOESNT_EXIST
import com.amaze.filemanager.file_operations.filesystem.WRITABLE_ON_REMOTE
...
class SmbUtilTest {
    ...
    fun testEncryptWithoutCredentials() {
        ...
        assertEquals(
            path, getSmbEncryptedPath(ApplicationProvider.getApplicationContext(), path)
        )
    }
    ...
    fun testCheckFolder() {
        assertEquals(
            DOESNT_EXIST, checkFolder("smb://user:password@5.6.7.8/newfolder/DummyFolder")
        )
        assertEquals(
            DOESNT_EXIST, checkFolder("smb://user:password@5.6.7.8/newfolder/resume.doc")
        )
        assertEquals(
            WRITABLE_ON_REMOTE,
        ...
        assertEquals(
            DOESNT_EXIST, checkFolder("smb://user:password@5.6.7.8/newfolder/failcheck")
        )
    }
