...
public class SmbConnectDialog extends DialogFragment {
    ...
    public interface SmbConnectionListener {
         ...
         *             now. See {@link com.amaze.filemanager.database.UtilsHandler(String,
         *             String)}
         */
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        @SuppressLint("InflateParams") final View v2 = getActivity().getLayoutInflater().inflate(R.layout.smb_dialog, null);
        ...
        conName.addTextChangedListener(
                new SimpleTextWatcher() {
                    ...
                    public void afterTextChanged(@NonNull Editable s) {
                        if (Objects.requireNonNull(conName.getText()).toString().length() == 0)
        ...
        ip.addTextChangedListener(
                new SimpleTextWatcher() {
                    @Override
                    public void afterTextChanged(@NonNull Editable s) {
                        if (Objects.requireNonNull(ip.getText()).toString().length() == 0)
        ...
        domain.addTextChangedListener(
                new SimpleTextWatcher() {
                    @Override
                    public void afterTextChanged(@NonNull Editable s) {
                        if (Objects.requireNonNull(domain.getText()).toString().contains(";"))
        ...
        user.addTextChangedListener(
                new SimpleTextWatcher() {
                    @Override
                    public void afterTextChanged(@NonNull Editable s) {
                        if (Objects.requireNonNull(user.getText()).toString().contains(":"))
        ...
        if (edit) {
            ...
            try {
                ...
                if (userinfo != null) {
                    ...
                    passp = inf.substring(inf.indexOf(":") + 1);
        ...
        ba3.onPositive(
                (dialog, which) -> {
                    String[] s;
                    String ipa = Objects.requireNonNull(ip.getText()).toString();
                    String con_nam = Objects.requireNonNull(conName.getText()).toString();
                    String sDomain = Objects.requireNonNull(domain.getText()).toString();
                    String sShare = Objects.requireNonNull(share.getText()).toString();
                    ...
                    if (chkSmbAnonymous.isChecked())
                        smbFile = createSMBPath(new String[]{ipa, "", "", domaind, sShare}, true);
                    else {
                        String useraw = user.getText().toString();
                        String useru = useraw.replaceAll(" ", "\\ ");
                        String passp = Objects.requireNonNull(pass.getText()).toString();
                        smbFile =
                                createSMBPath(new String[]{ipa, useru, passp, domaind, sShare}, false);
                    }
    ...
    private SmbFile createSMBPath(String[] auth, boolean anonymous) {
        try {
            ...
            return new SmbFile(
                    sb.toString(),
                    CifsContexts.createWithDisableIpcSigningCheck(sb.toString(), false));
        } catch (MalformedURLException e) {
            e.printStackTrace();
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
