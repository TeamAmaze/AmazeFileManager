...
public class TwilightCalculator {
  ...
  public int calculateTwilight(long time, double latitude, double longitude) {
    ...
    // true anomaly
    final float trueAnomaly =
        (float)
            (meanAnomaly
                + C1 * Math.sin(meanAnomaly)
    ...
    double solarTransitJ2000 =
        n + J0 + arcLongitude + 0.0053f * Math.sin(meanAnomaly) + -0.0069f * Math.sin(2 * solarLng);
    ...
    double cosHourAngle =
        (Math.sin(ALTITUDE_CORRECTION_CIVIL_TWILIGHT) - Math.sin(latRad) * Math.sin(solarDec))
