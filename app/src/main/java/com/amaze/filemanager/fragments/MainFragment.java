...
public class MainFragment extends android.support.v4.app.Fragment implements BottomBarButtonPath {
    ...
    public void reloadListElements(boolean back, boolean results, boolean grid) {
        ...
        if (adapter == null) {
            ...
            adapter.setItems(listView, new ArrayList<>(LIST_ELEMENTS), false);
        }
    ...
    public synchronized void addSearchResult(List<HybridFileParcelable> parcelables, String query) {
        ...
        if (listView != null) {
            ...
            ArrayList<LayoutElementParcelable> layoutElementParcelables = new ArrayList<>();
            for (HybridFileParcelable hybridFileParcelable : parcelables) {
                layoutElementParcelables.add(addTo(hybridFileParcelable));
            }
            if (!results) {
                adapter.setItems(listView, LIST_ELEMENTS, false);
                getMainActivity().getAppbar().getBottomBar().setPathText("");
                getMainActivity().getAppbar().getBottomBar().setFullPathText(getString(R.string.searching, query));
                results = true;
            } else {
                adapter.setItems(listView, layoutElementParcelables, true);
            }
