...
abstract class FtpClientTemplate<T>(url: String, closeClientOnFinish: Boolean = true) :
    NetCopyClientTemplate<T>(url, closeClientOnFinish) {

    @Throws(IOException::class)
    final override fun execute(client: NetCopyClient): T? {
        val ftpClient: FTPClient = client.getClientImpl() as FTPClient
     ...
     **/
    @Throws(IOException::class)
    abstract fun <T> executeWithFtpClient(ftpClient: FTPClient): T?
}
